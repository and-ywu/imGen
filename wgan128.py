...
f = h5py.File('data/128normalized.h5', 'r')
data = np.array(f['images'])

now = datetime.now()
timestamp = now.strftime("%Y%m%d%H%M%S")
os.makedirs("images/wgan128_{}".format(timestamp))
os.makedirs("models/wgan128_{}".format(timestamp))
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("images/wgan128_{}/{}.png".format(timestamp, epoch))
        plt.close()

wgan = WGAN()
wgan.train(epochs=20000, batch_size=32, sample_interval=1000)
wgan.generator.save_weights('models/wgan128_{}/wgangen.h5'.format(timestamp))
wgan.critic.save_weights('models/wgan128_{}/wgancritic.h5'.format(timestamp))
for i in range(5):
    imgs = wgan.generator.predict(np.random.normal(0, 1, (5, wgan.latent_dim)))
    fig = plt.figure(figsize=(10,10))
    plt.imshow(np.squeeze(imgs[i]), cmap='gray')
    plt.axis('off')
    fig.savefig("images/wgan128_{}/f{}.png".format(timestamp, i))
    plt.close()
