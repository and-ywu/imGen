...
f = h5py.File('data/256normalized.h5', 'r')
data = np.array(f['images'])

now = datetime.now()
timestamp = now.strftime("%Y%m%d%H%M%S")
os.makedirs("images/wgan256_{}".format(timestamp))
os.makedirs("models/wgan256_{}".format(timestamp))
bs = 8
    ...
    def sample_images(self, epoch):
        ...
        for i in range(r):
            for j in range(c):
                ...
                axs[i,j].axis('off')
                cnt += 1
        fig.savefig("images/wgan256_{}/{}.png".format(timestamp, epoch))
        plt.close()

wgan = WGAN()
wgan.train(epochs=15000, batch_size=bs, sample_interval=1000)
wgan.generator.save_weights('models/wgan256_{}/wgangen.h5'.format(timestamp))
wgan.critic.save_weights('models/wgan256_{}/wgancritic.h5'.format(timestamp))
for i in range(5):
    imgs = wgan.generator.predict(np.random.normal(0, 1, (5, wgan.latent_dim)))
    fig = plt.figure(figsize=(10,10))
    plt.imshow(np.squeeze(imgs[i]), cmap='gray')
    plt.axis('off')
    fig.savefig("images/wgan256_{}/f{}.png".format(timestamp, i))
    plt.close()
